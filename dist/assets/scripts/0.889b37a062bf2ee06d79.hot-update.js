	...
	var form = function () {
		...
		_hyperform2.default.setRenderer('attachWarning', function (warning, element) {
			if (element.getAttribute('type') === 'radio' || element.getAttribute('type') === 'checkbox') {
				element.closest('.control-group' || '.radio').after(warning);
			} else {
		...
		var bookmarkletForm = document.querySelector('.form[name=bookmarklet]');
		var registrForm = document.querySelector('.form[name=registr]');
		var recoveryForm = document.querySelector('.form[name=recovery]');
		var feedbackForm = document.querySelector('.form[name=feedback]');
		var claimForm = document.querySelector('.form[name=claim]');
		...
		if (bookmarkletForm) {
			var $bookmarkletForm = (0, _hyperform2.default)(bookmarkletForm, validatorClasses);
		}
		...
		if (recoveryForm) {
			var $formRecovery = (0, _hyperform2.default)(recoveryForm, validatorClasses);
		}
	
		if (feedbackForm) {
			var $formFeedback = (0, _hyperform2.default)(feedbackForm, validatorClasses);
		}
	
		if (claimForm) {
			var $formClaim = (0, _hyperform2.default)(claimForm, validatorClasses);
		}
	}();
...
//# sourceMappingURL=0.889b37a062bf2ee06d79.hot-update.js.map